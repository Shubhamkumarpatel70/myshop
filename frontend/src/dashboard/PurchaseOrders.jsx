            ...
            <div className="grid grid-cols-1 gap-4">
                {pos.map((po) => (
                    <motion.div 
                        ...
                        className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 flex flex-col md:flex-row justify-between items-center gap-6 group hover:border-indigo-500/30 transition-all"
                    >
                        <div className="flex items-center gap-6 w-full md:w-auto">
                            <div className="w-16 h-16 bg-slate-50 dark:bg-slate-800 rounded-2xl flex items-center justify-center text-indigo-600">
                                <FileText size={28} />
                            </div>
                            <div>
                                <div className="flex items-center gap-3">
                                    <h3 className="text-lg font-black tracking-tight">#{po.poNumber}</h3>
                                    <select 
                                        value={po.status}
                                        onChange={(e) => updateStatus(po._id, e.target.value)}
                                        className={`px-3 py-1 rounded-full text-[8px] font-black uppercase border-none cursor-pointer focus:ring-0 ${
                                ...
                                <p className="text-slate-500 font-bold text-sm mt-0.5">{po.supplierName || po.supplier?.name || 'Unknown Vendor'}</p>
                            </div>
                        </div>

                        <div className="flex items-center gap-12 w-full md:w-auto justify-between md:justify-end">
                            <div className="text-right hidden sm:block">
                                <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Expected By</p>
                                <p className="font-bold text-sm">{po.expectedDelivery ? new Date(po.expectedDelivery).toLocaleDateString() : 'ASAP'}</p>
                            </div>
