...
const AdminApprovals = () => {
    ...
    return (
        <div className="space-y-12 pb-32 font-jakarta">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-10">
                <div className="space-y-3">
                    ...
                    <h1 className="text-4xl md:text-7xl font-black tracking-tighter uppercase dark:text-white leading-[0.9]">
                ...
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-amber-500 text-white px-10 py-5 rounded-[2rem] text-sm font-black uppercase tracking-widest shadow-2xl shadow-amber-500/20 border-2 border-amber-400 rotate-2">
            ...
            {loading ? (
                <div className="grid grid-cols-1 gap-8">
                    {[1, 2, 3].map(i => <div key={i} className="h-40 bg-white dark:bg-slate-900 animate-pulse rounded-[3rem] border border-slate-100 dark:border-white/5"></div>)}
                </div>
            ) : pendingShops.length === 0 ? (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="h-[50vh] flex flex-col items-center justify-center text-center p-20 bg-white dark:bg-slate-900 rounded-[4rem] border-2 border-dashed border-slate-100 dark:border-white/5 shadow-2xl">
                    <div className="w-24 h-24 bg-emerald-500 rounded-[2.5rem] flex items-center justify-center text-white mb-8 shadow-2xl shadow-emerald-500/20 rotate-12">
                        <ShieldCheck size={48} />
                    </div>
                    <h3 className="text-4xl font-black uppercase tracking-tighter dark:text-white">Queue Clear</h3>
                            ...
                            <motion.div key={shop._id} layout initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: idx * 0.1 }} className="bg-white dark:bg-slate-900 p-10 rounded-[3.5rem] border border-slate-100 dark:border-white/5 shadow-xl hover:shadow-3xl hover:border-amber-500/30 transition-all group relative overflow-hidden">
                                ...
                                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-10 relative z-10">
                                    <div className="flex items-center gap-10">
                                        <div className="w-24 h-24 bg-amber-50 dark:bg-amber-500/10 text-amber-600 rounded-[2.5rem] flex items-center justify-center font-black text-4xl shadow-inner border border-amber-100 dark:border-amber-500/20 group-hover:rotate-6 transition-transform">
                                            {shop.shopName.charAt(0)}
                                        </div>
                                        <div>
                                            <h3 className="text-3xl font-black uppercase tracking-tight dark:text-white mb-3 group-hover:text-amber-500 transition-colors leading-none">{shop.shopName}</h3>
                                            <div className="flex flex-wrap items-center gap-x-8 gap-y-4">
                                                <div className="flex items-center gap-3 text-slate-400">
                                                    <User size={16} className="text-amber-500" /> <span className="text-[10px] font-black uppercase tracking-widest">{shop.ownerName}</span>
                                                </div>
                                                <div className="flex items-center gap-3 text-slate-400">
                                                    <FileText size={16} className="text-amber-500" /> <span className="text-[10px] font-black uppercase tracking-widest">{shop.businessType}</span>
                                                </div>
                                                <div className="flex items-center gap-3 text-slate-400">
                                                    <Calendar size={16} className="text-amber-500" /> <span className="text-[10px] font-black uppercase tracking-widest">{formatDate(shop.createdAt)}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-6 w-full lg:w-auto">
                                        <button onClick={() => { setSelectedShop(shop); setIsDetailModalOpen(true); }} className="flex-1 lg:flex-none h-18 px-10 bg-slate-950 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-amber-600 transition-all flex items-center justify-center gap-3">
                                            <Eye size={20} /> Review Details
                                        </button>
                                        <button onClick={() => handleStatusUpdate(shop._id, 'Approved')} disabled={submitting} className="flex-1 lg:flex-none h-18 px-10 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-700 transition-all flex items-center justify-center gap-3 shadow-2xl shadow-emerald-500/20">
