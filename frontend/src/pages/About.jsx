    ...
    const pillars = [
        {
            ...
            title: t('Unified Retail Stack'),
            description: t('Billing, inventory, customer tracking, and reporting work together in one platform.'),
        },
        {
            icon: <ShieldCheck className="h-6 w-6 text-indigo-600" />,
            title: t('Operational Reliability'),
            description: t('Built with role-based access and robust workflows to support daily store operations.'),
        },
        {
            icon: <BarChart3 className="h-6 w-6 text-indigo-600" />,
            title: t('Actionable Insights'),
            description: t('Clear dashboards help you make faster decisions on stock, sales, and profitability.'),
        },
    ];
        ...
        <main className="overflow-x-hidden bg-slate-50 pb-20 pt-32 text-slate-900 dark:bg-[#020617] dark:text-white">
            <section className="relative mx-auto max-w-7xl px-4 sm:px-6">
                ...
                <div className="mx-auto max-w-3xl text-center">
                    ...
                    >
                        {t('Built to help retail teams run faster and smarter every day')}
                    </motion.h1>
                    <motion.p
                        initial={{ opacity: 0, y: 14 }}
                        animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                        {t('StockSaathi was created for businesses that have outgrown manual processes and disconnected tools. We focus on practical workflows that reduce errors, save time, and improve decision-making.')}
                    </motion.p>
                </div>
                ...
                <div className="mt-14 grid gap-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)] dark:border-slate-700 dark:bg-slate-900 lg:grid-cols-2 lg:p-10">
                    <div>
                        <h2 className="font-outfit text-2xl font-bold tracking-tight sm:text-3xl">{t('Our mission')}</h2>
                        <p className="mt-4 text-sm leading-relaxed text-slate-600 sm:text-base dark:text-slate-300">
                            {t('Enable every store owner to operate with confidence through real-time visibility, reliable controls, and simple team collaboration.')}
                        </p>
                        <div className="mt-6 grid grid-cols-2 gap-3">
                            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50">
                                <p className="text-2xl font-bold">{t('1 Platform')}</p>
                                <p className="text-xs text-slate-500 dark:text-slate-400">{t('For billing + inventory + reports')}</p>
                            </div>
                            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50">
                                <p className="text-2xl font-bold">{t('Multi-user')}</p>
                                <p className="text-xs text-slate-500 dark:text-slate-400">{t('Owner, manager, cashier roles')}</p>
                            </div>
                        </div>
                    </div>
                    <div className="rounded-2xl bg-slate-950 p-6 text-white shadow-xl sm:p-8">
                        <div className="flex items-center gap-2 text-indigo-300">
                            ...
                            <p className="text-sm font-semibold">{t('Customer-first product direction')}</p>
                        </div>
                        <p className="mt-4 text-sm leading-relaxed text-slate-300 sm:text-base">
                            {t('We continuously improve the product around real retail use cases: faster checkouts, cleaner stock flow, and clearer business reporting.')}
                        </p>
